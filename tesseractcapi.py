...
class PsmEnum:
    ...
    PSM_CIRCLE_WORD ,PSM_SINGLE_CHAR ,PSM_SPARSE_TEXT ,PSM_SPARSE_TEXT_OSD ,PSM_COUNT = range(14)
...
class TesseractWrapper:
    ...
    def __init__(self, lang, libpath, tessdata):
        ...
        if (rc):
            ...
            exit(3)
        self.tesseract.TessBaseAPISetVariable(self.api, "tessedit_char_whitelist", "123456789")
    ...
    def imageFromNumpyImgToString(self, img, psm=None):
        # call SetImage
        if psm is not None:
        ...
        self.tesseract.TessBaseAPISetImage.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_ubyte), ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int]
        self.tesseract.TessBaseAPISetImage.restype = ctypes.c_void_p
        self.tesseract.TessBaseAPISetImage(self.api, img.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte)), 
            img.ctypes.shape[0], img.ctypes.shape[1], img.ctypes.strides[1], img.ctypes.strides[0])
...
def main():
    ...
    filepath = "/home/rory/Projects/sudoku-solver/debug_B3_9.jpg"
    for filename in glob.iglob('debug*.jpg'):
        print filename
        wrapper.imageFileToString(filename)
